...
All data now lives in the single CommunicationIQ database (no separate
``tenant_<slug>`` databases). Tenant isolation is by ``tenant_id`` field
on every document, queried at request time.
"""
...
class User(Document):
    """An institution user: tenant admin or student."""
...
class ReadingAttempt(Document):
    ...
    # The set's quiz-item ids this attempt covers (set-based practice).
...
class ExamReview(Document):
    """Student review of an exam attempt.

    Stored per-attempt. One review per student per attempt.
    Visible to the student, their tenant admin, and platform superadmin.
    """
...
# Question/content models — shared across ALL institutions (single pool).
...
class PracticeSitting(Document):
    """One student, one practice set, one sitting.

    The unit of practice is a Question Set (10 questions of one skill). A
    sitting records which set a student received so the rotation can serve the
    next unused set next time instead of a random one — no set twice until
    every other set has been seen. A sitting only counts as done once it was
    submitted; abandoned sittings release their set back to the rotation.
    """

    id: StrId = Field(default_factory=_uuid, alias="_id")
    tenant_id: str = Field(default="", index=True)
    user_id: str = Field(default="", index=True)
    skill: str = Field(default="", index=True)  # reading|listening|writing|speaking|quiz
    set_id: str = Field(default="", index=True)
    set_number: str = ""
    question_ids: list[str] = Field(default_factory=list)  # served order
    status: str = Field(default="open", index=True)  # open|completed
    score: float | None = None
    started_at: datetime = Field(default_factory=_now)
    completed_at: datetime | None = None

    class Settings:
        name = "practice_sittings"


class CompanyPaper(Document):
    """One company exam paper: 10 reading + 10 writing + 10 listening + 10 speaking.

    A company round is one paper of four sections, ten questions each, every
    question from that company's own bank — a general question never appears in
    a company paper, and a company paper never draws from another company. The
    paper references QuestionSets so the Question Bank stays the single place
    where content is organised.
    """
    ...
    reading_set_id: str = ""
    writing_set_id: str = ""
    listening_set_id: str = ""
    speaking_set_id: str = ""
    quiz_set_id: str = ""  # optional grammar section
...
TENANT_DOCUMENTS = [
    ...
    NotificationLog, StudentFlag, ExamReview, PracticeSitting,
]
