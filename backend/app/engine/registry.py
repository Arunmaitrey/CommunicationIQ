...
class Providers:
    ...
    async def resolve(self, capability: Capability, tenant_id: str | None = None) -> Resolution:
        ...
        # Deferred rather than raised here: an inactive primary and a primary
        # that fails to *load* (handled below) are the same situation from a
        # fallback's point of view, and only the load-failure case used to
        # reach the promotion logic -- a primary merely flagged inactive in
        # the registry raised immediately instead, so a configured fallback
