...
Platform staff read the shape of the business (tenants, seats), the shape of
the system (capabilities, providers, latency, audit), and the shared content
library (question bank, exam tests, exam schedules). Since the single-database
migration, institution data lives in the same database as everything else —
isolation is enforced by tenant_id filtering at the query level, not by a
separate database this file cannot reach, so no route here should read
attempt-, recording- or score-level student data without an explicit reason.
"""
...
async def overview() -> PlatformOverview:
    from app.models.platform import Plan as _Plan
    tenants = await Tenant.find_all().to_list()
    plans = await _Plan.find_all().count()
...
async def create_company(body: dict) -> dict:
    ...
    existing = await Company.find(Company.name == name).first_or_none()
    if existing:
...
# Email Templates
#
# Model, frontend page and lib/api.ts client all shipped in the merged PR;
# these CRUD routes did not -- the page called four endpoints that raised
# 404 against a router that never defined them. Added here, matching the
# ContactMessage CRUD immediately below as the established pattern.
# --------------------------------------------------------------------------

@router.get("/email-templates")
async def list_email_templates() -> list[dict]:
    from app.models.platform import EmailTemplate
    rows = await EmailTemplate.find_all().sort("name").to_list()
    return [
        {
...
async def list_exam_schedules() -> list[dict]:
    ...
    async def _attempt_stats(profile_id: str, starts_at, ends_at) -> dict:
        if not profile_id:
            return {"started": 0, "students": 0, "average_score": None}
        match = {"profile_id": profile_id,
                 "status": {"$nin": ["created"]},
                 "created_at": {"$gte": starts_at, "$lte": ends_at}}
        started = await db.attempts.count_documents(match)
        students = len(await db.attempts.distinct("user_id", match))
        avg = await db.attempts.aggregate([
            {"$match": match},
    ...
    for s in schedules:
        ...
        # Motor returns datetimes naive even though they were stored aware;
        # `now` is aware, so a bare comparison raises instead of ordering.
        starts_at = s.starts_at if s.starts_at.tzinfo else s.starts_at.replace(tzinfo=timezone.utc)
        ends_at = s.ends_at if s.ends_at.tzinfo else s.ends_at.replace(tzinfo=timezone.utc)
        status = ("ended" if now > ends_at
                  else "live" if now >= starts_at
                  else "upcoming")
        if not s.is_active:
        ...
        out.append({
            ...
            "results": await _attempt_stats(s.profile_id, s.starts_at, s.ends_at),
        })
    return out
...
async def list_question_sets(module: str = "") -> list[dict]:
    ...
    if module:
        query["module"] = module
    sets = await QuestionSet.find(query).sort("-created_at").to_list()
    return [
        {
...
async def assign_for_attempt(body: dict) -> dict:
    ...
    if assessment_id:
        test = await get_tolerant(ExamTest, assessment_id)
        if not test:
