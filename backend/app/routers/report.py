...
async def report_html(attempt_id: str, principal: Principal,
    ...
    # Get tenant info. logo_url lives inside Tenant.branding (a dict), not as
    # its own field — see app/routers/auth.py's _branding_fields, which reads
    # it the same way for the session payload.
    tenant_name = ""
    tenant_logo = None
    if principal.tenant_id:
        ...
        async with platform_sessionmaker()() as platform:
            ...
            if tenant:
                ...
                tenant_logo = (tenant.branding or {}).get("logo_url") or None
