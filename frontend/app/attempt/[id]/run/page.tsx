...
function Runner() {
  ...
  // ── Proctoring ──────────────────────────────────────────────────────
  // ProctorCamera (MediaPipe face/gaze detection + fullscreen + tab-switch)
  // owns its own violation state, persistence, and lock screen — it calls
  // back here only to auto-submit once the strike limit is reached.
  const examCompleted = phase === "submitting" || phase === "saving" || phase === "failed";
  const handleProctorAutoEnd = useCallback(async () => {
    if (phase === "submitting" || phase === "saving" || phase === "failed") return;
    try {
      setPhase("submitting");
      ...
      <Centered>
        <div className="max-w-sm text-center space-y-4">
          ...
          <p className="text-xs text-muted leading-relaxed">
            This exam is monitored and must be taken in fullscreen. On the
  ...
  return (
    <div className={`runner${skin ? ` ${skin.theme}` : ""}`}>
      {/* Proctoring: camera preview, face/gaze/fullscreen/tab checks, and
          the lock screen once the strike limit is hit. */}
      <ProctorCamera
        sessionId={id}
        enabled
        onViolation={() => {}}
