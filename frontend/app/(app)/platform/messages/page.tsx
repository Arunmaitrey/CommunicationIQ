...
interface ContactMessageReply { from: string; text: string; at: string }

interface ContactMessageRow {
  id: string; from_user_id: string; from_email: string; from_name: string;
  from_role: string; from_tenant_id: string | null;
...
function Messages() {
  ...
  const sendReply = async (id: string) => {
    ...
    try {
      ...
      toast("error", (e instanceof Error ? e.message : String(e)) || "Failed to send reply");
    }
    setReplying(false);
  };
            ...
            <div key={msg.id} className="ds-card">
              <button onClick={() => setExpanded(expanded === msg.id ? null : msg.id)}
                ...
                <span className="px-1.5 py-0.5 rounded text-[9px] font-medium" style={(() => {
                  const tone = msg.priority === "urgent" ? "var(--rag-red)"
                    : msg.priority === "high" ? "var(--rag-amber)" : "var(--muted)";
                  return { background: `color-mix(in srgb, ${tone} 15%, transparent)`, color: tone };
                })()}>
                  {msg.priority}
                </span>
