...
interface CompanyQuestion {
  ...
  title?: string; stem?: string; prompt_text?: string;
}
...
function Companies() {
  ...
  const [companyQuestions, setCompanyQuestions] = useState<Record<string, CompanyQuestion[]>>({});
  ...
  async function loadCompanyQuestions(companyName: string) {
    ...
    try {
      ...
      const d: Record<string, Omit<CompanyQuestion, "_type">[]> = await res.json();
      const all: CompanyQuestion[] = [
        ...(d.reading_passages || []).map((i) => ({ ...i, _type: 'Reading' })),
        ...(d.listening_passages || []).map((i) => ({ ...i, _type: 'Listening' })),
        ...(d.task_items || []).map((i) => ({ ...i, _type: 'Speaking' })),
        ...(d.writing_prompts || []).map((i) => ({ ...i, _type: 'Writing' })),
        ...(d.quiz_items || []).map((i) => ({ ...i, _type: 'Quiz' })),
      ];
                  ...
                  {loadingQuestions && !companyQuestions[company.name] ? (
                    ...
                    <>
                      ...
                      <div className="space-y-1 max-h-72 overflow-y-auto">
                        {(companyQuestions[company.name] || []).map((q, i: number) => (
