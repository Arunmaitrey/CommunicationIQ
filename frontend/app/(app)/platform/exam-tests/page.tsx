...
const SECTION_FIELDS: { key: NumericFormField; label: string; icon: typeof BookOpen; color: string }[] = [
...
const TIME_FIELDS: { key: NumericFormField; label: string }[] = [
...
interface ExamTestRow {
  ...
  is_active: boolean; is_baseline: boolean; company: string;
  question_ids: Record<string, string[]>;
  created_at?: string | null;
}

interface SetSummaryEntry { total_sets: number; active_sets: number; questions_available: number }
interface CompanySetSummaryEntry { active_sets: number; questions_available: number }

interface ExamTestForm {
...
type NumericFormField = {
  [K in keyof ExamTestForm]: ExamTestForm[K] extends number ? K : never
}[keyof ExamTestForm];

const EMPTY_FORM: ExamTestForm = {
...
function ExamTests() {
  ...
  const [tests, setTests] = useState<ExamTestRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<ExamTestRow | null>(null);
  const [setSummary, setSetSummary] = useState<Record<string, SetSummaryEntry>>({});
  const [companySetSummary, setCompanySetSummary] = useState<Record<string, Record<string, CompanySetSummaryEntry>>>({});
  const [form, setForm] = useState<ExamTestForm>(EMPTY_FORM);
  ...
  const set = <K extends keyof ExamTestForm>(k: K, v: ExamTestForm[K]) =>
  ...
  const toggleActive = async (t: ExamTestRow) => {
    const token = getToken();
    try {
      const body = {
  ...
  return (
    <>
      <PageHeader title="Exam Tests" sub={`${tests.length} tests configured`}
        action={<button onClick={() => { setShowForm(true); setEditing(null); setForm(EMPTY_FORM); }}
      ...
      {showForm && (
        <div className="ds-card p-5 mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {SECTION_FIELDS.map((sf) => (
                <div key={sf.key} className="p-2 rounded" style={{ background: "var(--surface-2)" }}>
                  ...
                  <input type="number" value={form[sf.key]}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {TIME_FIELDS.map((tf) => (
                <div key={tf.key}>
                  ...
                  <input type="number" value={form[tf.key]}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            ] as { k: keyof ExamTestForm & string; l: string }[]).map((f) => (
              <label key={f.k} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
                <input type="checkbox" checked={form[f.k] as boolean}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {[{ m: "reading", n: t.reading_questions },
                      { m: "listening", n: t.listening_questions },
                      { m: "writing", n: t.writing_questions },
                      { m: "speaking", n: t.speaking_questions },
                    ].filter(x => x.n > 0).map(({ m, n }) => {
                      const cs: Partial<CompanySetSummaryEntry> =
                        (t.company ? companySetSummary[t.company]?.[m] : undefined) ?? {};
