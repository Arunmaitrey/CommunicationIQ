    ...
    <span className="inline-flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          ...
          size={14}
          style={i <= rating
            ? { color: "var(--rag-amber)", fill: "var(--rag-amber)" }
...
function DifficultyBadge({ d }: { d: string }) {
  ...
  const tone = d === "easy" ? "var(--rag-green)" : d === "hard" ? "var(--rag-red)" : "var(--rag-amber)";
  return (
        ...
        {reviews.length === 0 ? (
          ...
          <Table
            ...
            rows={reviews.map((r: ReviewRow) => [
              ...
              r.tenant_id ? <span className="badge text-[10px]" style={{ background: "color-mix(in srgb, var(--secondary) 15%, transparent)", color: "var(--secondary)" }}>{r.tenant_id}</span> : "—",
              r.profile_name || "—",
