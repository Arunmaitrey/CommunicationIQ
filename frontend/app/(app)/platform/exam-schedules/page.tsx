...
import { CalendarClock, Plus, X, Trash2, Save, Clock, Users, Layers, Edit, Building2 } from "lucide-react";
...
function ExamSchedules() {
  ...
  const set = (k: keyof ScheduleForm, v: any) => setForm((prev) => ({ ...prev, [k]: v }));
  ...
  const openEdit = (s: any) => {
    ...
    const isAll = !s.institutions || s.institutions.length === 0
      || s.institutions.some((i: any) => i.id === "");
    setAllInstitutions(isAll);
    setForm({
      exam_test_id: s.exam_test_id,
      tenant_ids: isAll ? [] : (s.institutions ?? []).map((i: any) => i.id),
  ...
  const toggleActive = async (s: any) => {
    try {
      await api.updateExamSchedule(s.id, {
        exam_test_id: s.exam_test_id,
        tenant_ids: s.institutions?.some((i: any) => i.id === "") ? [] : (s.institutions ?? []).map((i: any) => i.id),
