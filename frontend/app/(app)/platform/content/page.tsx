...
import {
  ...
  ChevronDown, ChevronRight, ChevronLeft, AlertTriangle, Upload, Volume2, Loader2,
} from "lucide-react";
...
/** One row from any of the five content collections, normalised onto a
 *  single shape for this page's shared list/expand/delete UI. Every field
 *  below is optional because a reading passage has no `stem` and a quiz
 *  item has no `transcript` — the index signature covers whatever else a
 *  raw Mongo document carries through the `...i` spread that builds these. */
...
interface QuestionBankData {
  reading_passages?: ContentItem[];
  listening_passages?: ContentItem[];
  task_items?: ContentItem[];
  writing_prompts?: ContentItem[];
  quiz_items?: ContentItem[];
  counts?: Record<string, number>;
}

interface PromptAudioFile { name: string; ext: string; size: number }
...
function QuestionBank() {
  ...
  const [promptAudioFiles, setPromptAudioFiles] = useState<PromptAudioFile[]>([]);
  ...
  _allCompanies = Array.from(new Set(dbCompanies.map((c) => c.name)));
  ...
  const loadData = () => {
    ...
    fetch(`${API_BASE}/platform/companies`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    }).then(r => r.ok ? r.json() : []).then((c: { name: string; color: string; is_active: boolean }[]) => {
      setDbCompanies(c.filter((co) => co.is_active).map((co) => ({ name: co.name, color: co.color })));
    }).catch(() => {});
    fetch(`${API_BASE}/platform/questions`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
      .then((r) => { if (!r.ok) throw new Error("Failed to load"); return r.json(); })
      .then((d: QuestionBankData) => {
        setData(d);
        const grouped: Record<string, ContentItem[]> = {
          reading: (d.reading_passages || []).map((i) => ({ ...i, _source: "Reading", _collection: "reading" })),
          listening: (d.listening_passages || []).map((i) => ({ ...i, _source: "Listening", _collection: "listening" })),
          speaking: (d.task_items || []).map((i) => ({ ...i, _source: "Speaking", _collection: "task" })),
          writing: (d.writing_prompts || []).map((i) => ({ ...i, _source: "Writing", _collection: "writing" })),
          quiz: (d.quiz_items || []).map((i) => ({ ...i, _source: "Quiz", _collection: "quiz" })),
        };
        setItems(grouped);
        setLoading(false);
      })
    ...
    fetch(`${API_BASE}/platform/prompt-audio`, {
      ...
      .then((d: { files: PromptAudioFile[] }) => setPromptAudioFiles(d.files || []))
      .catch(() => {});
  };
  ...
  return (
    <>
      ...
      <div className="ds-card overflow-hidden mb-4">
        <div className="w-full flex items-center gap-3 p-4 hover:bg-surface2 transition-colors">
          <button onClick={() => setAudioExpanded(!audioExpanded)}
            className="flex-1 flex items-center gap-3 text-left">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: "color-mix(in srgb, var(--primary) 8%, transparent)" }}>
          ...
          <button onClick={() => setAudioExpanded(!audioExpanded)} className="shrink-0">
            ...
            {promptAudioFiles.length === 0 ? (
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-72 overflow-y-auto">
                  {promptAudioFiles.slice(audioPage * AUDIO_PAGE_SIZE, (audioPage + 1) * AUDIO_PAGE_SIZE).map((f) => (
...
interface AddQuestionForm {
  stem: string; category: string;
  options: string[]; correct_index: number; explanation: string;
  task_type: string; prompt_text: string; reference_text: string;
  audioKey: string; audioFile: File | null;
  title: string; kind: string; prompt: string; scenario: string;
  key_points: string[]; min_words: number; suggested_minutes: number;
  transcript: string; approx_seconds: number; plays_allowed: number;
  questions_text: string;
  body: string;
  company: string; difficulty: number;
}

const EMPTY_ADD_FORM: AddQuestionForm = {
  // quiz
...
function AddQuestionModal({ type, onClose, onCreated }: { type: string; onClose: () => void; onCreated: () => void; }) {
  ...
  const [error, setError] = useState("");
  const [form, setForm] = useState<AddQuestionForm>(EMPTY_ADD_FORM);

  const set = <K extends keyof AddQuestionForm>(k: K, v: AddQuestionForm[K]) =>
    setForm((f) => ({ ...f, [k]: v }));
  ...
  const handleSubmit = async () => {
    ...
    const token = getToken();
    try {
      const url = `${API_BASE}/platform/questions/${type}`;
      ...
      toast("error", (e instanceof Error ? e.message : String(e)) || "Failed to create question");
  ...
  const parseQuestions = (text: string) => {
    ...
    const questions: { stem: string; options: string[]; correct_index: number; explanation: string }[] = [];
...
interface ImportProblem { row: number; field: string; message: string; severity: string }
interface ImportPreviewRow { category: string; stem: string; options: string[]; difficulty: string; company: string }
interface ImportPreviewResult {
  ok: boolean; total: number; valid: number; warnings: number; errors: number;
  duplicates: number; detected_category: string;
  problems: ImportProblem[]; preview: ImportPreviewRow[];
}
interface ImportConfirmResult {
  ok: boolean; created: number; errors: number; total: number;
  by_category: Record<string, number>; message?: string;
}
...
function BulkUploadModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void; }) {
  ...
  return (
    <div className="modal-scrim" onClick={onClose}>
      <div className="modal-panel" style={{ maxWidth: "42rem" }}
        onClick={(e) => e.stopPropagation()}>
          ...
          <div className="space-y-4">
            ...
            {preview.problems?.length > 0 && (
              <div className="max-h-32 overflow-y-auto">
                ...
                {preview.problems.slice(0, 20).map((p, i: number) => (
            ...
            {preview.preview?.length > 0 && (
              <div>
                ...
                <div className="space-y-1 max-h-40 overflow-y-auto">
                  {preview.preview.map((row, i: number) => (
