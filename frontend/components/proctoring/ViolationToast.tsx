...
import { VIOLATION_COPY, MAX_VIOLATIONS } from "@/lib/proctoring/constants";
import { isDocumentFullscreen } from "@/lib/proctoring/detectors/fullscreenExit";
 ...
 * design tokens so it looks like part of the product rather than a
 * bolted-on component: warning icon, title, body copy, a "Violation X of N"
 * counter, and one wide primary button to acknowledge. The final violation
 * gets a red accent and a short violation summary instead of an
 * auto-dismiss timer.
 */
export default function ViolationToast({ violation, onDismiss, onReenterFullscreen }: ViolationToastProps) {
    ...
    >
      ...
      >
        ...
        {!isFinal && needsFullscreen ? (
          ...
          <button type="button" onClick={handleDismiss} className="btn btn-primary w-full mt-4">
            OK
          </button>
        )}
      </div>
