...
import { getProctoringStream, stopProctoringStream } from "@/lib/proctoringStream";
import {
  getConfirmDuration, MAX_VIOLATIONS, DETECTION_INTERVAL_MS,
  VIOLATION_COPY, VIOLATION_TYPES, type ViolationType,
} from "@/lib/proctoring/constants";
import { readPersistedState, writePersistedState } from "@/lib/proctoring/storage";
import { checkFaceNotDetected } from "@/lib/proctoring/detectors/faceNotDetected";
import { checkMultipleFaces } from "@/lib/proctoring/detectors/multipleFaces";
import { createLookingAwayTracker, type Landmark } from "@/lib/proctoring/detectors/lookingAway";
import { isDocumentFullscreen, requestFullscreen, exitFullscreen } from "@/lib/proctoring/detectors/fullscreenExit";
import {
  loadPhoneDetector, releasePhoneDetector, runObjectDetection, checkMobilePhone, countPersons,
} from "@/lib/proctoring/detectors/mobilePhone";
import ViolationToast, { type ActiveViolation } from "./ViolationToast";
import { API_BASE, getToken } from "@/lib/api";

interface AutoEndPayload {
  reason: string;
...
interface ProctorCameraProps {
  sessionId: string;
  enabled?: boolean;
  onViolation?: (violation: { type: ViolationType; count: number; timestamp: number }) => void;
  onAutoEnd?: (info: AutoEndPayload) => void;
  examCompleted?: boolean;
  inline?: boolean;
}

interface ActiveViolationTracking {
  type: ViolationType;
  since: number;
  confirmed: boolean;
}

// Disabled: too many false positives in practice (flags candidates who are
// reading the question or looking at their own typed answer). Off until the
// head-pose/gaze thresholds get retuned — see lib/proctoring/detectors/lookingAway.ts.
const LOOKING_AWAY_DETECTION_ENABLED = false;

interface FaceEvalState {
...
/**
 * ProctorCamera — AI exam proctoring component.
 *
 * Watches the camera feed for a missing/extra face, a look-away, or a phone
 * in view, and the document for a fullscreen exit or tab switch. Each
 * sustained violation is confirmed after its type's grace window (see
 * CONFIRM_DURATIONS_MS) and shown via ViolationToast; after MAX_VIOLATIONS + 1
 * confirmed violations the exam is locked. onAutoEnd fires once the candidate
 * dismisses the final-violation toast (or, on a page reload that lands
 * directly on the plain lock screen, once they click through it) rather than
 * the instant the lock happens — so the summary isn't yanked away mid-read.
 */
export default function ProctorCamera({
  sessionId, enabled = true, onViolation, onAutoEnd, examCompleted = false, inline = false,
}: ProctorCameraProps) {
  ...
  const activeViolationRef = useRef<ActiveViolationTracking | null>(null);
  const lookingAwayTrackerRef = useRef<ReturnType<typeof createLookingAwayTracker> | null>(null);
  if (!lookingAwayTrackerRef.current) {
    lookingAwayTrackerRef.current = createLookingAwayTracker();
  }

  const phoneSessionRef = useRef<Awaited<ReturnType<typeof loadPhoneDetector>> | null>(null);
  const phoneCheckInFlightRef = useRef(false);
  const activePhoneViolationRef = useRef<{ since: number; confirmed: boolean } | null>(null);

  // Person-body count from the same YOLO pass used for phone detection. Fed
  // into checkMultipleFaces() as a fallback for a second person whose face
  // isn't frontal (turned away / side-on), which MediaPipe's face-only
  // detector can't see on its own. Defaults to 1 until the first
  // object-detection pass completes.
  const personCountRef = useRef(1);

  const pendingAutoEndRef = useRef<AutoEndPayload | null>(null);

  const persistedOnMount = useMemo(() => readPersistedState(sessionId), [sessionId]);

  const [faceStatus, setFaceStatus] = useState("Initializing...");
  const [currentToast, setCurrentToast] = useState<ActiveViolation | null>(null);
  ...
  const [violationCount, setViolationCount] = useState(() => persistedOnMount?.violationCount ?? 0);
  ...
  const faceEvalRef = useRef<FaceEvalState>({
    face_detected: false, away_events: 0, multi_face_events: 0, violation_count: 0,
  });
  ...
  const teardown = useCallback(() => {
    ...
    // The exam is over one way or another whenever teardown runs (finished,
    // locked out, or the page is going away) — leaving the browser stuck in
    // fullscreen after that would be a UX dead end, not a proctoring signal.
    exitFullscreen();
  }, []);
  ...
  }, [sessionId, examCompleted]);

  // ==================================================
  // 1. INITIALIZE MEDIAPIPE + PHONE DETECTOR
  // ==================================================
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        try {
          ...
          // Non-fatal: mobile-phone and improved person-count detection are
          // an enhancement, not a requirement — proctoring still runs on the
          // face/fullscreen/tab checks alone if the YOLO model fails to load.
          console.warn("Mobile-phone detection unavailable:", err);
        }
  ...
  // ==================================================
  const confirmViolation = useCallback((type: ViolationType) => {
    ...
    writePersistedState(sessionId, undefined, {
      locked: isFinal,
    ...
    if (isFinal) {
      // Release the camera hardware immediately rather than waiting for the
      // isLocked-driven effect — on a backgrounded tab (exactly the case
      // when the user has switched to another app), React can delay that
      // effect, leaving the camera indicator on longer than it should be.
      // teardown() is idempotent (safe to call again from the effect right
      // after).
      teardown();
      setIsLocked(true);
  ...
  }, [sessionId, onViolation, teardown]);

  const trackViolation = useCallback((type: ViolationType, now: number) => {
  ...
  const trackPhoneViolation = useCallback((now: number) => {
  ...
  useEffect(() => {
    ...
    const detect = () => {
      ...
      if (now - lastDetectionTimeRef.current >= DETECTION_INTERVAL_MS) {
        ...
        // independently of the synchronous face checks below. Guarded so a
        // slow inference call can't stack up multiple overlapping runs. The
        // same detection pass feeds both checkMobilePhone() and the
        ...
        try {
          ...
          if (noFace) {
            ...
            const faceLandmarks = landmarkResult?.faceLandmarks?.[0] as Landmark[] | undefined;
            ...
            if (!faceLandmarks) {
              ...
              const lookingAway = LOOKING_AWAY_DETECTION_ENABLED
                ? lookingAwayTrackerRef.current?.check(faceLandmarks) ?? null
                : null;
              if (lookingAway) {
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      ...
      // rather than trusting an internal "already warned" flag that never
      // resets — a second exit (via tab switch, going back and returning,
      // etc.) would otherwise silently stop being detected.
  ...
  const statusTone = useCallback((): "ok" | "warn" | "error" | "neutral" => {
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }}>
        ...
        >
          ...
          <h2 className="text-base font-bold" style={{ color: "var(--text)" }}>Fullscreen Required</h2>
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }}>
        ...
        >
          ...
          <h2 className="text-base font-bold" style={{ color: "var(--text)" }}>Proctoring Unavailable</h2>
          <p className="mt-2 text-[13px] leading-relaxed" style={{ color: "var(--muted)" }}>
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }}>
        ...
        >
          ...
          <h2 className="text-base font-bold" style={{ color: "var(--text)" }}>Interview Locked</h2>
          <p className="mt-2 text-[13px] leading-relaxed" style={{ color: "var(--muted)" }}>
          ...
          <button
            type="button"
            onClick={() => {
              const payload: AutoEndPayload = pendingAutoEndRef.current || {
                reason: "proctoring_violation_limit_reached",
                violationType: (persistedOnMount?.lastViolationType as ViolationType | undefined) ?? null,
      ...
      {!inline && (
        <div className="fixed bottom-4 right-4 z-50">
          <div className="bg-white dark:bg-slate-900 border-2 border-gray-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden">
            <video ref={videoRef} autoPlay muted playsInline className="w-40 h-28 object-cover bg-black rounded-t-xl" />
