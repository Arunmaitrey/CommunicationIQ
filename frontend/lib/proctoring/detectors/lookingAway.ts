...
const HEAD_ENTER = { horizontal: 0.35, down: 0.35, up: -0.15 };
const HEAD_EXIT = { horizontal: 0.25, down: 0.25, up: -0.08 };
const GAZE_ENTER = { horizontal: 0.18, vertical: 0.18 };
const GAZE_EXIT = { horizontal: 0.12, vertical: 0.12 };
const SMOOTHING_ALPHA = 0.35;

function averageLandmarks(landmarks: Landmark[], indices: number[]): Landmark | null {
...
function getNormalizedEyePosition(
  iris: Landmark | null,
  left: Landmark,
  right: Landmark,
  top: Landmark,
  ...
  if (eyeWidth < 0.005 || eyeHeight < 0.005) return null;
...
export function getHeadPose(landmarks: Landmark[] | null | undefined) {
...
export function getEyeGaze(landmarks: Landmark[] | null | undefined) {
  ...
  const leftEye = getNormalizedEyePosition(
    leftIris, landmarks[33], landmarks[133], landmarks[159], landmarks[145],
  );
  const rightEye = getNormalizedEyePosition(
    rightIris, landmarks[362], landmarks[263], landmarks[386], landmarks[374],
  );
...
function getHeadDirection(horizontal: number, vertical: number): string {
...
function getGazeDirection(horizontal: number, vertical: number): string {
...
function getAttentionDirection(headDirection: string, gazeDirection: string): string {
...
export function createLookingAwayTracker() {
  ...
  return {
    ...
    checkDetailed(faceLandmarks: Landmark[] | null | undefined) {
    ...
    check(faceLandmarks: Landmark[] | null | undefined): ViolationType | null {
      return this.checkDetailed(faceLandmarks).violation;
    },
  };
}

export function checkLookingAway(faceLandmarks: Landmark[] | null | undefined): ViolationType | null {
  const headPose = getHeadPose(faceLandmarks);
  const eyeGaze = getEyeGaze(faceLandmarks);
