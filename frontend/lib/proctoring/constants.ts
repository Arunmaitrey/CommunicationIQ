...
// backgrounded, not after a grace window.
export const CONFIRM_DURATIONS_MS: Record<string, number> = {
...
export const getConfirmDuration = (type: string): number =>
  CONFIRM_DURATIONS_MS[type] ?? CONFIRM_DURATION_MS;

export const VIOLATION_COPY: Record<string, { title: string; body: string }> = {
